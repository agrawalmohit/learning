...
public class RomanToInt {
	...
	public static int romanToInt(String roman) {
		...
		for (int i = 0; i < roman.length(); i++) {
			String rom = roman.substring(i, i + 1);
			if (!last.isEmpty() && romanMap.containsKey(last + rom)) {
				num = num - romanMap.get(last);
				num = num + romanMap.get(last + rom);
			} else {
				num = num + romanMap.get(rom);
			}
			last = rom;
		}
		return num;
